...
# ─────────────────────────────────────────────
#  Dataset：Breaking Bad 真实数据（适配真实目录结构）
#
#  目录结构：
#    data_root/
#    └── everyday/
...
class BreakingBadPairDataset(Dataset):
    ...
    每条样本对应一个 fracture_* / mode_* 子目录。
    __getitem__ 将该目录下所有碎片的顶点合并为完整点云，
    再施加随机刚体变换，构成 (points1, points2, transform) 三元组。

    参数
    ----
    data_root : str
        包含 everyday/ 子目录的数据集根路径。
        若直接传入 everyday/ 本身亦可正常工作。
    category : str
        类别名称，需与 everyday/ 下的实际目录名一致，例如 'BeerBottle'。
    split : str
        'fracture' → 只使用 fracture_* 目录（默认）
        'mode'     → 只使用 mode_* 目录
        'all'      → 两者都使用
    num_points : int
        每个点云采样的目标点数。
    ...
    def __init__(self,
        ...
        self.data_root = candidate if candidate.exists() else base
        ...
        if len(self.samples) == 0:
            raise RuntimeError(
                ...
                f"请确认路径正确，且存在 fracture_* / mode_* 子目录及 piece_*.obj 文件。"
            )
        print(f"✓ 找到 {len(self.samples)} 个 {category} 样本（split={split}）")

    # ── 扫描：每个 fracture_* / mode_* 目录作为一条样本 ──────
    def _scan_samples(self) -> List[Dict]:
        ...
        for shape_dir in sorted(category_dir.iterdir()):
            ...
            for frac_dir in sorted(shape_dir.iterdir()):
                if not frac_dir.is_dir():
                    continue

                name = frac_dir.name  # e.g. 'fracture_0' / 'mode_3'

                # 按 split 过滤
                if self.split == 'fracture' and not name.startswith('fracture'):
                    continue
                if self.split == 'mode' and not name.startswith('mode'):
                    continue
                # split == 'all'：两者都保留
                ...
                samples.append({
                    ...
                    'piece_files': [str(p) for p in piece_files],
                })
    ...
    def __getitem__(self, idx):
        ...
        # ── 合并该破碎目录下所有碎片的顶点 ──────────────────────
        all_vertices = []
        for pf in info['piece_files']:
            mesh  = o3d.io.read_triangle_mesh(pf)
        ...
        if len(all_vertices) == 0:
            # 极端情况：所有文件损坏，返回零点云
            combined = np.zeros((self.num_points, 3), dtype=np.float32)
        else:
            combined = np.vstack(all_vertices)   # (N_total, 3)

        # ── 采样到固定点数 ────────────────────────────────────────
        N = len(combined)
        if N >= self.num_points:
            idx_sel = np.random.choice(N, self.num_points, replace=False)
        else:
            # 不足则有放回补采
            extra   = np.random.choice(N, self.num_points - N, replace=True)
            idx_sel = np.concatenate([np.arange(N), extra])
        points = combined[idx_sel]               # (num_points, 3)
        ...
        points1 = points
        points2 = (R @ points.T + t.reshape(3, 1)).T
...
def train_breaking_bad(config_path: str, output_dir: str, data_root: str,
    ...
    print("\n" + "=" * 60)
    print(f"使用 Breaking Bad 数据集训练 ({category} / split={split})")
    print("=" * 60)
    ...
    print(f"[数据] 加载 Breaking Bad 数据集（{data_root}）...")
...
def parse_args():
    ...
    parser.add_argument('--predator_config', type=str,
                        default='configs/predator.yaml')
    parser.add_argument('--dcp_config', type=str,
                        default='configs/dcp.yaml')
    parser.add_argument('--output_dir', type=str,
                        default='pretrained_weights')
    parser.add_argument('--data_root', type=str,
                        default=r'D:\googledownload\Breaking-Bad-Dataset.github.io-main'
                                r'\Breaking-Bad-Dataset.github.io-main',
                        help='包含 everyday/ 子目录的数据集根路径')
    parser.add_argument('--category', type=str, default='BeerBottle',
                        choices=['BeerBottle', 'Bottle', 'Bowl', 'Mug', 'Vase'],
                        help='类别名称（需与 everyday/ 下的实际目录名一致）')
    parser.add_argument('--split', type=str, default='fracture',
                        choices=['fracture', 'mode', 'all'],
                        help='使用 fracture_* / mode_* 目录，或全部')
...
def main():
    ...
    if args.mode is None:
        print("\n可用训练模式:")
        print("  predator      - 训练 Predator 模型（合成/真实数据）")
        print("  dcp           - 训练 DCP 模型（合成数据）")
        print("  breaking_bad  - 使用 Breaking Bad 数据集训练 Predator")
        print("  both          - 同时训练 Predator 和 DCP（合成数据）")
        args.mode = input("\n请选择训练模式: ").strip().lower()
