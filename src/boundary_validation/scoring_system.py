...
class ScoringSystem:
    ...
    def _extract_component_scores(self, feature_result: Any, complementarity_result: Any,
                                 alignment_result: Any, collision_result: Any) -> Dict[str, float]:
        """
        提取各个组件的原始得分
        """
        scores = {}
        ...
        if feature_result is not None:
            scores['feature_score'] = feature_result.boundary_complementarity_score
        else:
            scores['feature_score'] = 0.0
            
        # 2. 法向互补性得分
        if complementarity_result is not None:
            scores['normal_score'] = complementarity_result.normal_complementarity_score
        else:
            scores['normal_score'] = 0.0
            
        # 3. 形状互补性得分
        if complementarity_result is not None:
            scores['shape_score'] = complementarity_result.shape_complementarity_score
        else:
        ...
        if alignment_result is not None:
            ...
            rmse_score = 1.0 / (1.0 + alignment_result.rmse * 100)  # RMSE越小得分越高
            scores['alignment_score'] = 0.7 * fitness_score + 0.3 * rmse_score
        else:
            scores['alignment_score'] = 0.0
            
        # 5. 碰撞惩罚得分（注意：这是惩罚项，值越小越好）
        if collision_result is not None:
            # 碰撞惩罚：得分越低惩罚越大
            collision_penalty = collision_result.collision_score
            # 转换为惩罚得分（0表示完全惩罚，1表示无惩罚）
            scores['collision_penalty'] = 1.0 - collision_penalty
        else:
            scores['collision_penalty'] = 0.0  # 默认完全惩罚
    ...
    def _determine_validation_status(self, total_score: float, 
                                   component_scores: Dict[str, float]) -> str:
        """
        确定验证状态
        """
        min_total = self.thresholds['minimum_total_score']
        min_component = self.thresholds['minimum_component_score']
        
        # 检查各组件最低要求
        component_failures = []
        for component, score in component_scores.items():
            if score < min_component and component != 'collision_penalty':  # 碰撞惩罚是反向指标
                component_failures.append(component)
        
        # 状态判定逻辑
        if total_score >= 0.85 and len(component_failures) == 0:
            return 'excellent_match'
        elif total_score >= 0.75 and len(component_failures) <= 1:
            return 'good_match'
        elif total_score >= 0.65 and len(component_failures) <= 2:
            return 'acceptable_match'
        elif total_score >= min_total and len(component_failures) <= 3:
